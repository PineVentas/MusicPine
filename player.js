        ...
        try {
            ...
            if (autoplaySetting?.autoplay) {
                ...
                if (!nextTrack) {
                    await cleanupTrackMessages(client, player);
                    player.destroy();
                    await channel.send("⚠️ **No hay más pistas para reproducir automáticamente. Desconectando...**");
                }
            } else {
                await cleanupTrackMessages(client, player);
                console.log(`Autoplay is disabled for guild: ${guildId}`);
                player.destroy();
                await channel.send("🎶 **La cola ha finalizado. La reproducción automática está desactivada..**");
            }
        } catch (error) {
            console.error("Error handling autoplay:", error);
            await cleanupTrackMessages(client, player);
            player.destroy();
            await channel.send("👾**¡Cola vacía! Desconectando...**");
        }
    });
}
...
async function cleanupPreviousTrackMessages(channel, guildId) {
    ...
    for (const messageInfo of messages) {
        try {
            ...
            console.error("Mensaje de error al limpiar la pista anterior:", error);
        }
    }
...
async function handleInteraction(i, player, channel) {
    switch (i.customId) {
        ...
        case 'skipTrack':
            player.stop();
            await sendEmbed(channel, "⏭️ **Cancion Saltada!**");
            break;
        case 'disableLoop':
        ...
        case 'clearQueue':
            player.queue.clear();
            await sendEmbed(channel, "🗑️ **La cola ha sido despejada!**");
            break;
        case 'stopTrack':
            player.stop();
            player.destroy();
            await sendEmbed(channel, '⏹️ **La reproducción ha sido detenida y el reproductor destruido!**');
            break;
        case 'pauseTrack':
            if (player.paused) {
        ...
        case 'resumeTrack':
            if (!player.paused) {
                ...
                player.pause(false);
                await sendEmbed(channel, '▶️ **Se ha reanudado la reproducción!**');
            }
            break;
        case 'volumeUp':
...
function toggleLoop(player, channel) {
    ...
    sendEmbed(channel, player.loop === "track" ? "🔁 **El bucle de seguimiento está activado!**" : "🔁 **El bucle de cola está activado!**");
}

function disableLoop(player, channel) {
    player.setLoop("none");
    sendEmbed(channel, "❌ **El bucle está deshabilitado!**");
}
